...
A prova disto é mostrada abaixo. Em relação a primeira equação, tem-se:

$$
\sum (X - \overline{X})(Y - \overline{Y}) = \sum X(Y - \overline{Y}) - \overline{X} \sum(Y - \overline{Y})
$$

como o desvio da média é zero

$$
\sum(Y - \overline{Y}) = 0
$$

a equação fica reduzida para 

$$
\sum (X - \overline{X})(Y - \overline{Y}) = \sum X(Y - \overline{Y})
$$

No caso da segunda equação, tem-se

$$
\sum X(X - \overline{X}) = \sum (X^2 - \overline{X}X)
$$

$$
\sum X(X - \overline{X}) = \sum X^2 - \sum \overline{X}X
$$

Como $(\overline{X})$ é a média de $X$, uma constante, tem-se:

$$
\sum X(X - \overline{X}) = \sum X^2 - \overline{X} \sum X
$$

Além disso, considerando a parte a direita da igualdade, 

$$
\sum (X - \overline{X})^2 = \sum (X^2 - 2X\overline{X} + \overline{X}^2)
$$


$$
\sum (X - \overline{X})^2 = \sum X^2 - 2\overline{X}\sum X + \sum \overline{X}^2
$$

Sendo $(\overline{X})$ uma constante, $(\sum \overline{X}^2)$ é igual a $(n\overline{X}^2)$, onde $(n)$ é a quantidade de termos:

$$
\sum (X - \overline{X})^2 = \sum X^2 - 2\overline{X}\sum X + n\overline{X}^2
$$

Como $(n\overline{X}^2 = \overline{X}\sum X)$ pois $(\overline{X} = \frac{\sum X}{n})$, então:

$$
\sum (X - \overline{X})^2 = \sum X^2 - 2\overline{X}\sum X + \overline{X}\sum X
$$

Simplificando

$$
\sum (X - \overline{X})^2 = \sum X^2 - \overline{X}\sum X
$$

Exatamente o que foi obtido anteriormente para $(\sum X(X - \overline{X}))$.

Continuando, então, a demostração de como encontrar a fórmula de cálculo do $\beta_1$, desde que $\sum(X-\overline{X})^2>0$, a inclinação estimada é
...
A equação dada em acima é a covariância amostral de x e y dividida pela variância amostral de x. Pode-se reescrever a forma de encontrar o $\hat{\beta}_1$ como abaixo:
...
result.get_prediction(nova_obs).summary_frame(alpha=0.05)
```
