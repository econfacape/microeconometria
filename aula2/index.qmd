...
O que está indicado na equação acima é que o valor médio de u é o mesmo para qualquer valor de x e igual a sua média aritmética. Em outras palavras, diz que u é **média-independente** de x: o valor esperado de u não muda conforme x varia.
...
![Modelo Clássico de Regressão Linear](img_r/fig_eyx_linear.png){width=85%}

Fonte: elaboração própria, com base em WOOLDRIDGE, 2017
...
![Modelo Clássico de Regressão Linear](img_r/fig_valores_residuos.png){width=85%}
...
Expandindo os termos dentro do somatório:

$$
\sum(X)(Y-\overline{Y})+\hat{\beta_1}\sum(X)(\overline{X}-X)=0
$$
...
A equação dada em acima é a covariância amostral de x e y dividida pela variância amostral de x. Podemos reescrever a forma de encontrar o $\hat{\beta}_1$ multiplicando e dividindo por $\hat{\sigma}_y$:
...
```{python econ6, warning=FALSE, message=FALSE}
import wooldridge
import pandas as pd
import numpy as np
