...
setwd('C:/Users/Lenovo/Dropbox/tempecon/Facape/econometria1')
...
```{python econ1a, message=FALSE, warning=FALSE}
#Pacotes a serem utilizados
import wooldridge
import pandas as pd
import numpy as np
...
reg1 = smf.ols('wage ~ exper', data=wage1)
...
reg2 = smf.ols('np.log(wage) ~ np.log(exper)', data=wage1)
#Resultado da Regressão
result_log = reg2.fit()
...
# Possibilidade 1 - Retirar da base educ=0
...
# Possibilidade 1 - Retirar da base educ=0
dados1 = wage1[wage1['educ'] != 0]
print(dados1["educ"].describe())

#Resultado da Regressão
reg3 = smf.ols('np.log(wage) ~ np.log(educ)', data=dados1)
result_log3 = reg3.fit()
...
# Possibilidade 2 - alterar o valor quando wage=0
dados2 = wage1
dados2['educ'] = dados2['educ'].replace(0, 0.000001)

#Estatística Descritiva de Educ
print(dados2["educ"].describe())

#Resultado da Regressão
reg4 = smf.ols('np.log(wage) ~ np.log(educ)', data=dados2)
result_log4 = reg4.fit()
print(result_log4.summary())
```
<br>
...
```{python econ4a, message=FALSE, warning=FALSE}

#Resultado da Regressão
reg5 = smf.ols('wage ~ np.log(educ)', data=dados2)
result_log5 = reg5.fit()
print(result_log5.summary())
```
...
```{python econ5a, message=FALSE, warning=FALSE}
#Resultado da Regressão Log-Lin

reg6 = smf.ols('np.log(wage) ~ educ', data=dados2)
result_log6 = reg6.fit()
print(result_log6.summary())
```
...
#Calculo da Taxa de Crescimento Geometrica
coefs = reg6.fit().params
print((np.exp(coefs['educ'])-1)*100)
```
