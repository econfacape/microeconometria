...
```{r setup}
library(reticulate)
use_condaenv("py-econo", require = TRUE)
py_config()
```

``` {r aula10_1, warning=FALSE, message=FALSE}
#Direcionado o R para o Diretorio a ser trabalhado
setwd('C:/Users/Lenovo/Dropbox/tempecon/facape')
...
Desta forma obtêm-se os erros-padrões robustos dos parâmetros. Fazendo uso do pacote `` sandwich``, e a opção para a matriz de variância-covariância consistente com heterocedasticidade e autocorrelação ``vcov=NeweyWest`` conforme o script abaixo. Conforme a ordem da autocorelação (se acima da primeira ordem), pode ser mais indicado considerar o tratamento para a estrutura geral de Newey e West (1987) e Wooldridge (1989), citados por Wooldridge
(2016, seção 12.5).
...
os.chdir('C:/Users/Lenovo/Dropbox/tempecon/facape')
...
resid = result1.resid
df_resid = pd.DataFrame({'u': resid})

max_lags = 10
rows = []

for lags in range(1, max_lags + 1):
    lm_stat, lm_pvalue, f_stat, f_pvalue = acorr_breusch_godfrey(result1, nlags=lags)
    rows.append({
        'Lags': lags,
        'LM-stat': lm_stat,
...
for p in range(1, max_lag + 1):
    df_aux = df_resid.copy()
    
    # cria as defasagens dos resíduos
    for j in range(1, p + 1):
        df_aux[f'u_lag{j}'] = df_aux['u'].shift(j)
    
    df_aux = df_aux.dropna()
    
    y = df_aux['u']
    X = df_aux[[f'u_lag{j}' for j in range(1, p + 1)]]
    X = sm.add_constant(X)

    model_p = sm.OLS(y, X).fit()
    
    aic_list.append(model_p.aic)
    bic_list.append(model_p.bic)
    
    print(f"p = {p} | AIC = {model_p.aic:.4f} | BIC = {model_p.bic:.4f}")

```


Há evidência de autocorrelação de $1^a$ e de $2^a$ ordem nos resíduos do modelo. A partir do 3 lag não se rejeita mais a hipótese nula. um processo AR(1) já captura bem a estrutura da autocorrelação dos resíduos, se for considerado o critério de informação. 


## Correlação da autocorrelação no Python

```{python}
result_hac1 = result1.get_robustcov_results(
    cov_type='HAC',
    maxlags=1  # número de defasagens na correção Newey-West
)

print(result_hac1.summary())
```

Mantém o modelo original, mas usa-se erros-padrão robustos Newey-West para corrigir o efeito da autocorrelação. Esta é uma solução prática e muito usada na literatura aplicada.
